...
const CENTRAL_LOGS_DIR = path.join(process.cwd(), "..", "..", "spectacular-loop-logs");
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  try {
    ...
    fs.symlinkSync(
      logsDir, 
      path.join(CENTRAL_LOGS_DIR, `iter-${iteration}-${path.basename(logsDir)}`),
      "dir"
    );
    ...
    const errorLogPath = path.join(CENTRAL_LOGS_DIR, `error-iter-${iteration}.json`);
    fs.writeFileSync(
      errorLogPath,
      JSON.stringify({
