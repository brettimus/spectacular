...
export function evaliteTracerPlugin(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      if (
        (id.includes("evalite") && id.includes("ai-sdk")) ||
        ...
        if (
          ...
          code.includes("traceAISDKModel")
        ) {
          console.log(
            `[evalite-tracer-plugin] Replacing traceAISDKModel in ${id}`,
          );
        ...
        if (
          ...
          console.log(
            `[evalite-tracer-plugin] Updating traceAISDKModel import in ${id}`,
          );
