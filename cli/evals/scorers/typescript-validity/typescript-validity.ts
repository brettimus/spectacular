 ...
 */
export const TypeScriptValidity = createScorer<
  ...
  try {
    ...
    try {
      ...
      const result = await execPromise(
        `cd ${projectDir} && npm run typecheck -- ${validationFile}`,
      );
      ...
      try {
        ...
        const fallbackResult = await execPromise(
          `cd ${projectDir} && npx tsc --noEmit ${validationFile}`,
        );
