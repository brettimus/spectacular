...
export function SpecificationsPanel({ projectId }: SpecificationsPanelProps) {
  ...
  const fetchSpecifications = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/specifications/project/${projectId}`);
      if (!response.ok) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this specification version?")) return;
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label htmlFor="content" className="text-sm font-medium">Content (JSON)</label>
              <Textarea
                id="content"
                name="content"
                ...
                <Card key={spec.id} className="overflow-hidden">
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-semibold">Version {spec.version}</h3>
